...
def prepare_dataset(tokenizer, max_length=128):
    ...
    def adjust_labels(example):
        example['label'] = float(example['label'] ) / 4  # Labels are from 0-4; convert to 0.0-1.0
        return example
...
def main():
    ...
    # Add an adapter and a regression head to the model
    ...
    model.add_classification_head(
        "yelp_head",
        num_labels=1,  # Yelp Review Full has labels from 0 to 4 (5 classes)
        # id2label={0: "1 star", 1: "2 stars", 2: "3 stars", 3: "4 stars", 4: "5 stars"},
    )

    # Activate the adapter and regression head
    ...
    training_args = TrainingArguments(
        output_dir="./gpt2-yelp-adapter_regression",
    ...
    # Save the adapter and the regression head
    adapter_save_path = "./saved_adapters/yelp_adapter_reg"
